...
def load_gui(self):
    ...
    self.lbl_phone.grid(row=6, column=0, padx=(27, 0), pady=(0, 0), sticky=N + W)
    ...
    self.txt_email.grid(row=5,column=0,rowspan=1,columnspan=2,padx=(30,40),pady=(0,0),sticky=N+E+W)
    self.txt_phone = tk.Entry(self.master, text='')
    self.txt_phone.grid(row=7,column=0,rowspan=1,columnspan=2,padx=(30,40),pady=(0,0),sticky=N+E+W)

    # Listbox to store our saved contacts
    self.scrollbar1 = Scrollbar(self.master, orient=VERTICAL)
    self.lstList1 = Listbox(self.master, exportselection=0, yscrollcommand=self.scrollbar1.set)
    # Create event when the user selects an object from the list
